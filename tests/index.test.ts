...
describe('logging', () => {
  ...
  fetch: mockTokenFetch,
    ...
    expect(client.logLevel).toBe('warn');
    expect(warnMock).toHaveBeenCalledWith('process.env[\'FRAGMENT_LOG\'] was set to "not a log level", expected one of ["off","error","warn","info","debug"]');
  });
  ...
  clientSecret: 'My Client Secret',
    ...
    test('empty env variable', () => {
      ...
      const client = new Fragment({ clientID: 'My Client ID', clientSecret: 'My Client Secret' });
      expect(client.baseURL).toEqual('https://api.us-west-2.fragment.dev/billing')
    });
