...
describe('resource transactions', () => {
  ...
  test.skip('list: request options and params are passed correctly', async () => {
    ...
    await expect(client.transactions.list({ account: 'ext_account_YWJjMTIz', reconciliation_status: 'reconciled' }, { path: '/_stainless_unknown_path' }))
      .rejects
  ...
  test.skip('searchAllocations: only required params', async () => {
    const responsePromise = client.transactions.searchAllocations({ filter: { invoice_id: { any: ['inv_abc123'] } } });
    const rawResponse = await responsePromise.asResponse();
    expect(rawResponse).toBeInstanceOf(Response);
    const response = await responsePromise;
    expect(response).not.toBeInstanceOf(Response);
